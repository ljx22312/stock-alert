#!/usr/bin/env python3
"""数据同步推送端：把本地采集的行情数据推送到数据服务网关（/ingest）。

用法:
  python3 jobs/sync_data.py stocks        # 推送股池 + 指数目录到 stocks 集合
  python3 jobs/sync_data.py backfill      # 一次性全历史日线回填（股池 + 指数）
  python3 jobs/sync_data.py daily         # 增量日线同步（拉最近 150 根 upsert，幂等）
  python3 jobs/sync_data.py hours         # 60分钟线同步（腾讯 mkline，滚动窗口约 4 个月）
  python3 jobs/sync_data.py snapshot      # 盘中最新快照推送（覆盖式）+ 分时历史帧 ticks
  python3 jobs/sync_data.py signals       # 增量信号日志推送
  python3 jobs/sync_data.py profile       # 日内量能分布（同时段5分钟均量基线）
  python3 jobs/sync_data.py all           # stocks + daily + hours + signals + snapshot + profile

配置: 仓库根 cloud_sync.json {"url": "...", "token": "..."}（不入库）
状态: 仓库根 cloud_sync.state.json（信号增量水位，自动维护）
"""
...
from monitor.quotes import fetch_quotes, to_market_code  # noqa: E402
from monitor.daily import TENCENT_URL, TENCENT_MKLINE_URL, _fetch_tencent, _fetch_eastmoney  # noqa: E402
...
# 只上网页、不参与本地监控的股票（本地有完整数据，网关补全历史后仅供网页展示）
WEB_ONLY = [
...
BATCH = 200  # 单次推送最大文档数
MAX_BYTES = 80 * 1024  # 单次推送请求体上限（HTTP 访问服务 ~100KB）
HISTORY_START = "2015-01-01"  # 回填起点


def load_cfg() -> dict:
    cfg = json.loads((ROOT / "cloud_sync.json").read_text())
...
# 增量同步水位（记录各标的已推送到网关的位置，避免每天全量覆盖重写）
STATE_DIR = ROOT / "data" / "sync_state"
# 水位初始化回读的 api 基址；可用 cloud_sync.json 的 "api_base" 覆盖（默认本机数据服务）
API_BASE = "http://127.0.0.1:8791/api"
...
def cloud_rows(kind: str, symbol: str, cfg: dict | None = None) -> list[dict]:
    """读网关当前已入库的行情（用于初始化增量水位；读比写便宜 10 倍）。
    kind: 'daily' | 'hour'。返回升序文档数组。数据源基址取 cfg.api_base，缺省本机数据服务。"""
...
def cmd_daily(cfg: dict) -> int:
    """增量日线同步：拉最近 150 根前复权，与网关水位对比，只推新增/复权调整过的根。

    前复权价在除权除息日会整体调整，故水位按 {date: close} 记录：
    非除权日每天仅新增 1 根（~36 次写），除权日一次性补推调整段。
    首次运行（无水位）从网关读回当前库建水位，不做全量覆盖。
    """
    ...
    for t in all_targets():
        ...
        if t["symbol"] not in meta:
            # 首次：读网关已入库的历史建水位（读 300 根 < 写 150 根 x 36 标的）
...
def cmd_hours(cfg: dict) -> int:
    ...
    只推超过水位的新 bar（每标的每天约 4-8 根）。首次运行读网关建水位。"""
...
def cmd_profile(cfg: dict) -> int:
    import sqlite3
    db = sqlite3.connect(str(ROOT / "data" / "stock_alert.db"))
...
def cmd_signals(cfg: dict) -> int:
    ...
    if rows:
        ...
        (ROOT / "cloud_sync.state.json").write_text(json.dumps(state))
    print(f"signals: 推送 {n} 条（水位 {state['last_signal_id']}）")
    return n


def main() -> None:
    ap = argparse.ArgumentParser(description="stock-alert 数据同步推送端")
